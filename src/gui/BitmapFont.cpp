...
// 8x8 pixel font, inspired by Atari ST system font style.
// ASCII 32 (space) through 127 (DEL). Each char = 8 bytes (top row first).
const uint8_t BitmapFont::fontData[96][8] = {
    ...
    {0x66,0x6C,0x18,0x30,0x6C,0xCC,0x00,0x00}, // 37 '%'
    {0x38,0x6C,0x38,0x76,0xDC,0x6C,0x36,0x00}, // 38 '&'
    ...
    {0xFC,0xC6,0xC6,0xC6,0xC6,0xC6,0xFC,0x00}, // 68 'D'
    ...
    {0xC6,0xC6,0xC6,0x6C,0x6C,0x38,0x10,0x00}, // 86 'V'
    ...
    {0xC0,0x60,0x30,0x18,0x0C,0x06,0x00,0x00}, // 92 '\'
    ...
    {0x00,0x00,0x7C,0xC0,0xC0,0xC6,0x7C,0x00}, // 99 'c'
...
void BitmapFont::drawText(juce::Graphics& g, const juce::String& text,
                          int x, int y, int scale, juce::Colour colour)
{
    ...
    {
        int ch = static_cast<int>(text[i]);
        if (ch < 32 || ch > 127) ch = 32;
        const uint8_t* glyph = fontData[ch - 32];
        ...
        for (int row = 0; row < CHAR_H; ++row)
        {
            ...
            {
                if (bits & (0x80 >> col))
                    g.fillRect(curX + col * scale, y + row * scale, scale, scale);
            }
        }
...
int BitmapFont::textWidth(const juce::String& text, int scale)
{
